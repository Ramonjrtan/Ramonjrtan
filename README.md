# 👋 Hi, I'm Ramon Tan Jr

🎯 Senior QA Engineer | 20+ Years Experience  
Specializing in **End-to-End System Validation, Payments, and Enterprise Systems**

---

## 🚀 What I Do

I test **how systems behave in real-world conditions** — not just UI.

✔ End-to-end workflows (UI → API → backend → data)  
✔ Payment & transaction systems  
✔ API & integration validation  
✔ Data integrity & reconciliation  
✔ Production risk detection  

---

# ⭐ Featured Projects (Start Here)

---

## 🚀 Enterprise Order-to-Cash QA (MOST COMPLETE)

🔗 https://github.com/Ramonjrtan/enterprise-order-to-cash-qa-portfolio  

✔ CRM → Orders → Payments → Accounting → Reconciliation  
✔ 84 test cases (Excel execution suite)  
✔ Financial + integration validation  

---

## 💰 FinTech Digital Wallet

🔗 https://github.com/Ramonjrtan/fintech-wallet-testing-liquidpay  

✔ Wallet transaction integrity  
✔ Balance validation  
✔ Failure / retry scenarios  

---

## 💳 POS Transaction Flow (ISO8583)

🔗 https://github.com/Ramonjrtan/pos-transaction-flow-simulation-iso8583-style  

✔ Authorization → reversal → settlement  
✔ Payment lifecycle validation  

---

## 🏦 Core Banking QA (T24)

🔗 https://github.com/Ramonjrtan/T24-QA-Portfolio  

✔ Maker-checker workflows  
✔ Financial data integrity  

---

## 🧠 Event-Driven Platform QA

🔗 https://github.com/Ramonjrtan/event-driven-order-platform-qa  

✔ API → events → DB validation  
✔ Idempotency and consistency  

---

# 🧩 System Integration & POS

- 🔗 https://github.com/Ramonjrtan/mobile-pos-linebuster-qa  
- 🔗 https://github.com/Ramonjrtan/pos-system-integration-testing-qa  

✔ Device → backend → POS  
✔ Real-world transaction workflows  

---

# 💳 Payments, Data & Reconciliation

- 🔗 https://github.com/Ramonjrtan/retail-payments-data-qa-portfolio-  
- 🔗 https://github.com/Ramonjrtan/qa-casino-platform-testing-portfolio  

✔ ETL + reconciliation  
✔ Financial reporting validation  

---

# 🛒 SaaS / E-commerce

- 🔗 https://github.com/Ramonjrtan/saas-conversion-funnel-qa-portfolio  
- 🔗 https://github.com/Ramonjrtan/shopify-ecommerce-qa-testcases  

✔ User journey + payment flows  
✔ Conversion and checkout validation  

---

# 🔌 API & Backend

- 🔗 https://github.com/Ramonjrtan/api-testing-portfolio-restful-booker  

✔ REST API validation  
✔ Auth, error handling  

---

# 🤖 Automation

- 🔗 https://github.com/Ramonjrtan/cypress-saas-e2e-automation-framework  
- 🔗 https://github.com/Ramonjrtan/salesforce-playwright-framework  

✔ UI automation frameworks  
✔ Scalable test architecture  

---

# ⚙️ IoT / Embedded / Smart Systems

- 🔗 https://github.com/Ramonjrtan/smart-metering-qa-portfolio  
- 🔗 https://github.com/Ramonjrtan/iot-embedded-qa-portfolio  
- 🔗 https://github.com/Ramonjrtan/telematics-fleet-management-qa-portfolio  

✔ Device → communication → backend validation  
✔ Real-time + distributed systems  

---

# 🏥 Enterprise Systems

- 🔗 https://github.com/Ramonjrtan/salesforce-manual-test-cases  
- 🔗 https://github.com/Ramonjrtan/openmrs-healthcare-qa-portfolio  
- 🔗 https://github.com/Ramonjrtan/workday-hris-testing-portfolio  

✔ Business workflows + data validation  
✔ Enterprise process testing  

---

# 🧪 My QA Approach

I focus on:
- End-to-end system behavior  
- Data integrity across layers  
- Financial and transaction accuracy  
- Real-world failure scenarios  

👉 I test **what actually breaks in production**

---

# 📫 Connect

- LinkedIn: https://www.linkedin.com/in/ramon-t-91946710/  
- Email: ramonjrtan@gmail.com  

---

⭐ QA portfolio focused on **real-world systems, not just test cases**
